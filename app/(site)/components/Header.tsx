    ...
    <LazyMotion features={domAnimation}>
      <header
        className={`sticky z-50 top-0 left-0 ${
          yScroll > 50 ? "shadow-md py-3" : "py-4"
        } px-4 w-full bg-secondary border-b border-gray-400 text-basic-text`}
      >
          ...
          <nav className="hidden lg:inline-flex">
            <ul className="flex items-center gap-8">
              {navLinks.map((link, index) => (
                <li
                  key={index}
                  className="relative flex items-center text-base font-semibold group"
                >
                  <Link
                    href={link.href}
                    onMouseEnter={handleMouseEnter(link.hasMenu)}
                    ...
                    <div className="absolute left-0 hidden mb-2 transition-opacity duration-300 bg-white rounded-lg shadow-lg w-60 top-full group-hover:block -z-10 group-hover:z-[100]">
                      <ul className="">
                        {productsType.map((type, index) => (
                          <li
                            key={index}
                            className="text-sm duration-300 hover:bg-primary hover:pl-2"
                          >
                            <Link
